...
Ported from: Satusehat\Integration\Terminology\FhirR4term
...
class FhirR4term(TerminologyBase):
    """
    FHIR R4 CodeSystem terminology entries.

    Ported from Laravel Eloquent model FhirR4term.
    Table: fhir_r4_term.
    """

    table_name: str = "fhir_r4_term"
    columns: list[str] = [
        "id",
        "fhir_id",
        "url",
    ...
    def create_table(cls) -> None:
        """Create the fhir_r4_term table."""
        conn = cls.get_connection()
        conn.execute("""
            CREATE TABLE IF NOT EXISTS fhir_r4_term (
    ...
    @classmethod
    def find_by_code(cls, code: str) -> Optional[FhirR4term]:
        """Find FhirR4term entry by fhir_id."""
        return super().find_by_code(code)  # type: ignore[return-value]

    @classmethod
    def find_by_url(cls, url: str) -> list[FhirR4term]:
        """Find all entries for a given CodeSystem URL."""
        conn = cls.get_connection()
        rows = conn.execute(
            "SELECT * FROM fhir_r4_term WHERE url = ?", (url,)
        ).fetchall()
